...
//         BitStream* encoded = new BitStream(argv[argc - 1], "w+");

//         //assert(encoded.Write(desc));

//         videoFile.ReadFrame(prevY, prevCb, prevCr);
//         videoFile.ReadFrame(Y, Cb, Cr);

//         MotionCompensation::Result result = MotionCompensation::Encode(prevY, Y, blockSize, searchArea);
//         MotionCompensation::Write(result, *encoded);

//         delete encoded;
//         BitStream* encoded2 = new BitStream(argv[argc - 1], "r");
        
//         uint64_t yBlocks = Y.rows / blockSize;
//         uint64_t xBlocks = Y.cols / blockSize;

//         cv::Mat tmp(Y.rows, Y.cols, CV_8UC1);
//         MotionCompensation::Decode(*encoded2, prevY, tmp, blockSize);

//         cv::imwrite("Original.bmp", Y);
//         cv::imwrite("Decoded.bmp", tmp);

//         std::cout << "Need " << result.Estimate() << " bits\n";
//         delete encoded2;